package com.example.cjj.mynews.dpullrefresh_loadmore;
 ...
 */
public class DPullRefreshLayout extends LinearLayout {

    public static final String Tag = DPullRefreshLayout.class.getSimpleName();
    private DRefreshViewHolder mRefreshViewHolder;
     ...
     * 下拉刷新和上拉加载更多代理
     */
    private DPullRefreshLayoutDelegate mDelegate;
    /**
     * 手指按下时，y轴方向的偏移量
     */
    ...
    public DPullRefreshLayout(Context context) {
        this(context, null);
    }

    public DPullRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new RuntimeException(DPullRefreshLayout.class.getSimpleName() + "必须有且只有一个子控件");
        }
    ...
    public void setRefreshViewHolder(DRefreshViewHolder refreshViewHolder) {
                ...
                mAbsListView.setOnScrollListener(new AbsListView.OnScrollListener() {
                    ...
                    public void onScrollStateChanged(AbsListView absListView, int scrollState) {
                        if ((scrollState == SCROLL_STATE_IDLE || scrollState == SCROLL_STATE_FLING)
                                && shouldHandleAbsListViewLoadingMore(mAbsListView)) {
                            beginLoadingMore();
                        }
    ...
    private boolean shouldHandleRefresh() {
        if (!mPullDownRefreshEnable || mIsLoadingMore ||
                mCurrentRefreshStatus == RefreshStatus.REFRESHING
                || mRefreshHeaderView == null || mDelegate == null) {
            return false;
        }
    ...
    public void beginRefreshing() {
        if (mCurrentRefreshStatus != RefreshStatus.REFRESHING && mDelegate != null) {
            ...
            handleRefreshStatusChanged();
            mDelegate.onDPullRefreshLayoutBeginRefreshing(this);
        }
    }
    ...
    public void beginLoadingMore() {
        if (!mIsLoadingMore && mLoadMoreFooterView != null && mDelegate != null &&
                mDelegate.onDPullRefreshLayoutBeginLoadingMore(this)) {
            mIsLoadingMore = true;
     ...
     * @param delegate
     */
    public void setDelegate(DPullRefreshLayoutDelegate delegate) {
        mDelegate = delegate;
    }

    public interface DPullRefreshLayoutDelegate {
        /**
         * 开始刷新
         */
        void onDPullRefreshLayoutBeginRefreshing(DPullRefreshLayout refreshLayout);
         ...
         */
        boolean onDPullRefreshLayoutBeginLoadingMore(DPullRefreshLayout refreshLayout);
    }
